...
Tank.prototype = {
    ...
    onData: function(socket, data) {
        ...
        if (!this._users.has(id)) {
            ...
            user.quota = this._normalWaterQuota;
            user.socket = socket;
        ...
        console.info("> Received", response, "from", id);

        // Check faucet reading with the maximum threshold for water usage
        user.waterUsage = response.usage;
        if (user.waterUsage >= user.quota && response.state !== Valve.state.CLOSED) {
            console.info("Stopping faucet at " + id + " with quota " + user.quota + " and usage " + user.waterUsage);
